...
Displays a [background](../4.graphics/1.backgrounds.md) image on the lower screen. The background image can only be of type `TEX_BG`.
...
This command displays an emote in a speech bubble above a [chibi](../4.graphics/3.chibis.md) figure on the top screen.
...
Specifies a [chibi](../4.graphics/3.chibis.md) figure to enter or exit the top screen. To be used by this command, a chibi must have
...
Displays a line of dialogue and/or manipulates [character sprites](../4.graphics/2.sprites.md). Can optionally play a [voiced line](../5.sound/2.voice.md).
...
Displays a particular "kinetic" (my word) [background](../4.graphics/1.backgrounds.md) on the top screen. Must be of type `KINETIC_SCREEN`.
