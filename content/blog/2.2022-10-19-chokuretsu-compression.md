...
head:
  ...
  - name: 'og:image:alt'
    value: 'A Nintendo DS featuring Haruhi Suzumiya saying edited text.'
  - name: 'og:url'
...
* `CMP R3, #0 ; BEQ loc_20262A0` – `BEQ`{lang="arm"} means “branch if equal,” but really it just means “branch if the last comparison is equal to zero.” Therefore, if that value we just loaded is zero, we’re going to branch to the end of the subroutine. We can ignore this for now.
* `TST R3, #0x80` – `TST`{lang="arm"} performs a bitwise-and without storing the result. A bitwise-and compares two bytes and gives a result where each bit is 1 only if that bit is 1 in both of the two bytes it compares. In the case where R3 is 0xAA, we end up with something like:
```
10101010 (0xAA)
10000000 (0x80)
_______
...
* `TST R3, #0x20` ; `ANDEQ R12, R3, #0x1F`{lang="arm"} – Now we’re testing the third bit of our first byte here. If it’s zero, we’re going to take its last five bits (0x1F = 0xb0001_1111) and branch to 0x2026280. We’ll get there in a sec.
...
* `CMP R12, #0` ; `BLE loc_2026198`{lang="arm"} – If the value we just calculated is zero, we immediately return to the top of the function.
...
* `B loc_2026198`{lang="arm"} – If it’s less than or equal to 0, we go back to the top of the subroutine.{lang="arm"}
